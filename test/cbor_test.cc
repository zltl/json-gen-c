...
#define ROUNDTRIP_PACK_UNPACK(Type) \
    ASSERT_EQ(0, cbor_pack_##Type(&src, buf)); \
    ASSERT_GT(sstr_length(buf), (size_t)0); \
    ASSERT_EQ(0, cbor_unpack_##Type( \
...
TEST(CborScalar, NegativeValues) {
    ROUNDTRIP_INIT(Scalar);
    src.i = -2147483647;
...
TEST(CborOptional, PresentFields) {
...
TEST(CborOptional, AbsentFields) {
    ROUNDTRIP_INIT(WithOptional);
    src.id = 42;
    src.has_name = false;
    src.has_score = false;
...
TEST(CborNullable, NonNullValues) {
    ROUNDTRIP_INIT(WithNullable);
    src.id = 2;
    src.has_name = true;
...
TEST(CborOneof, CircleRoundTrip) {
...
TEST(CborOneof, RectangleRoundTrip) {
    ROUNDTRIP_INIT(Drawing);
...
TEST(CborArrayPack, StructArray) {
    ...
    sstr_t buf = sstr_new();
    ASSERT_EQ(0, cbor_pack_array_Scalar(items, 2, buf));
    ASSERT_GT(sstr_length(buf), (size_t)0);

    struct Scalar *out = NULL;
    int count = 0;
    ASSERT_EQ(0, cbor_unpack_array_Scalar(
...
TEST(CborFormat, SmallPositiveInt) {
    /* cbor positive fixint: 0x00-0x7f */
    ROUNDTRIP_INIT(Scalar);
    src.i = 42;

    ASSERT_EQ(0, cbor_pack_Scalar(&src, buf));
...
TEST(CborFormat, NilEncoding) {
    /* CBOR null: 0xf6 (major 7, additional info 22) */
    ROUNDTRIP_INIT(WithNullable);
    src.id = 1;
    src.has_name = false;
    src.has_value = false;

    ASSERT_EQ(0, cbor_pack_WithNullable(&src, buf));

    const unsigned char *bytes = (const unsigned char *)sstr_cstr(buf);
    size_t len = sstr_length(buf);
    int nil_count = 0;
    for (size_t j = 0; j < len; j++) {
        if (bytes[j] == 0xf6) nil_count++;
    }
    EXPECT_GE(nil_count, 2) << "Expected at least 2 CBOR null bytes for null name and value";
...
TEST(CborEdge, EmptyMapRoundTrip) {
    ROUNDTRIP_INIT(WithMap);
...
TEST(CborEdge, InvalidData) {
    struct Scalar s;
    Scalar_init(&s);

    /* Completely invalid data */
    unsigned char bad[] = {0xff, 0xff, 0xff};
    int r = cbor_unpack_Scalar(bad, 3, &s);
    EXPECT_NE(r, 0) << "Should fail on invalid data";

    /* Empty data */
    r = cbor_unpack_Scalar(NULL, 0, &s);
