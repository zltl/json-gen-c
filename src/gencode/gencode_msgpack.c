/**
 * @file gencode_msgpack.c
 * @brief Generate MessagePack pack/unpack C code from parsed schema.
 *
 * Parallel to gencode.c (JSON) but emits binary MessagePack codec
 * using the msgpack_codec.h/c embedded runtime.
 */
...
static void mp_map_c_value_type(struct struct_field *field, sstr_t out) {
...
static const char *mp_map_suffix(struct struct_field *field) {
...
static void mp_gen_struct_header(struct struct_container *st, sstr_t header) {
    ...
    while (field) {
        if (field->type == FIELD_TYPE_MAP) {
            ...
            mp_map_c_value_type(field, suffix);
            sstr_printf_append(
                header,
                ...
                "};\n"
                "#endif\n\n",
                mp_map_suffix(field), mp_map_suffix(field),
                mp_map_suffix(field), sstr_cstr(suffix),
                mp_map_suffix(field), mp_map_suffix(field));
            sstr_free(suffix);
        }
        field = field->next;
    ...
    while (field) {
        ...
        if (field->type == FIELD_TYPE_MAP) {
            sstr_printf_append(header, "struct map_container_%s %s%s",
                               mp_map_suffix(field), ptr_suffix, sstr_cstr(field->name));
    ...
    sstr_printf_append(
        ...
        "int msgpack_pack_%s(struct %s *obj, sstr_t out);\n"
        "int msgpack_unpack_%s(const unsigned char *data, size_t len, struct %s *obj);\n"
        "int msgpack_pack_array_%s(struct %s *obj, int count, sstr_t out);\n"
        "int msgpack_unpack_array_%s(const unsigned char *data, size_t len, struct %s **obj, int *count);\n\n",
...
static void mp_gen_enum_header_fn(void *key, void *value, void *ptr) {
    (void)key;
...
static void mp_gen_enum_string_fn(void *key, void *value, void *ptr) {
    (void)key;
    struct enum_container *ec = (struct enum_container *)value;
...
static void mp_gen_oneof_header(struct oneof_container *oc, sstr_t header) {
    /* tag enum */
...
static void mp_gen_oneof_tag_strings(struct oneof_container *oc, sstr_t source) {
...
static void mp_gen_oneof_statics_fn(void *key, void *value, void *ptr) {
    (void)key;
    struct oneof_container *oc = (struct oneof_container *)value;
    sstr_t source = (sstr_t)ptr;
    mp_gen_oneof_tag_strings(oc, source);
}

/* ── init / clear (shared logic, same as JSON) ──────────────────────── */

static void mp_gen_struct_init(struct struct_container *st, sstr_t source) {
...
static void mp_gen_clear_map_entries(struct struct_field *field,
...
static void mp_gen_struct_clear(struct struct_container *st, sstr_t source) {
    sstr_printf_append(source,
        "int %s_clear(struct %s *obj) {\n",
        sstr_cstr(st->name), sstr_cstr(st->name));
    ...
    while (f) {
        ...
        if (f->type == FIELD_TYPE_SSTR && !f->is_array) {
            ...
            mp_gen_clear_map_entries(f, base, source);
            ...
            if (f->type == FIELD_TYPE_SSTR) {
                ...
                mp_gen_clear_map_entries(f, idx_expr, source);
                sstr_append_cstr(source, "    }\n");
            }
            sstr_printf_append(source,
                "    JGENC_FREE(%s);\n"
...
static void mp_gen_oneof_init(struct oneof_container *oc, sstr_t source) {
    sstr_printf_append(source,
        "int %s_init(struct %s *obj) {\n"
        "    memset(obj, 0, sizeof(struct %s));\n"
        "    return 0;\n}\n\n",
...
static void mp_gen_oneof_clear(struct oneof_container *oc, sstr_t source) {
    sstr_printf_append(source,
        "int %s_clear(struct %s *obj) {\n"
        "    switch (obj->tag) {\n",
        sstr_cstr(oc->name), sstr_cstr(oc->name));
...
static void mp_gen_pack_scalar(struct struct_field *f, const char *accessor,
                               sstr_t source) {
    switch (f->type) {
    case FIELD_TYPE_INT:
    ...
    case FIELD_TYPE_INT32:
        sstr_printf_append(source, "    mp_pack_int(out, (int64_t)%s);\n", accessor);
        break;
    ...
    case FIELD_TYPE_UINT32:
        sstr_printf_append(source, "    mp_pack_uint(out, (uint64_t)%s);\n", accessor);
        break;
    case FIELD_TYPE_UINT64:
        sstr_printf_append(source, "    mp_pack_uint(out, (uint64_t)%s);\n", accessor);
        break;
    case FIELD_TYPE_FLOAT:
        sstr_printf_append(source, "    mp_pack_float(out, %s);\n", accessor);
        break;
    case FIELD_TYPE_DOUBLE:
        sstr_printf_append(source, "    mp_pack_double(out, %s);\n", accessor);
        break;
    case FIELD_TYPE_BOOL:
        sstr_printf_append(source, "    mp_pack_bool(out, %s);\n", accessor);
        break;
    case FIELD_TYPE_SSTR:
        sstr_printf_append(source, "    mp_pack_sstr(out, %s);\n", accessor);
        break;
    case FIELD_TYPE_ENUM:
        sstr_printf_append(source,
            "    if (%s >= 0 && %s < %s_enum_str_count) {\n"
            "        const char *_es = %s_enum_strings[%s];\n"
            "        mp_pack_str(out, _es, (uint32_t)strlen(_es));\n"
            "    } else {\n"
            "        mp_pack_int(out, (int64_t)%s);\n"
            "    }\n",
            accessor, accessor, sstr_cstr(f->type_name),
            sstr_cstr(f->type_name), accessor, accessor);
        break;
    case FIELD_TYPE_STRUCT:
        sstr_printf_append(source, "    msgpack_pack_%s(&%s, out);\n",
...
/* Pack map value */
static void mp_gen_pack_map_value(struct struct_field *field,
...
static void mp_gen_pack_struct(struct struct_container *st, sstr_t source) {
    ...
    if (has_optional) {
        ...
        sstr_append_cstr(source, "    mp_pack_map_header(out, (uint32_t)_nfields);\n");
    } else {
        sstr_printf_append(source,
            "    mp_pack_map_header(out, %d);\n", total_fields);
    }
    ...
    while (f) {
        ...
        if (f->type == FIELD_TYPE_MAP && !f->is_array) {
            /* map field */
            sstr_printf_append(source,
                "    mp_pack_map_header(out, (uint32_t)%s.len);\n"
                "    for (int _mk = 0; _mk < %s.len; _mk++) {\n"
                "        mp_pack_sstr(out, %s.entries[_mk].key);\n",
                accessor, accessor, accessor);
            char val_acc[300];
            snprintf(val_acc, sizeof(val_acc), "%s.entries[_mk].value", accessor);
            mp_gen_pack_map_value(f, val_acc, source);
            sstr_append_cstr(source, "    }\n");
        } else if (f->is_array) {
            int len_known = (f->array_size > 0);
            if (len_known) {
                sstr_printf_append(source,
                    "    mp_pack_array_header(out, %d);\n"
                    "    for (int _ai = 0; _ai < %d; _ai++) {\n",
                    f->array_size, f->array_size);
            } else {
                sstr_printf_append(source,
                    "    mp_pack_array_header(out, (uint32_t)obj->%s_len);\n"
            ...
            elem_f.is_array = 0;
            mp_gen_pack_scalar(&elem_f, elem_acc, source);
            sstr_append_cstr(source, "    }\n");
        } else {
            mp_gen_pack_scalar(f, accessor, source);
        }
...
static void mp_gen_unpack_scalar(struct struct_field *f, const char *accessor,
                                 sstr_t source) {
    switch (f->type) {
    case FIELD_TYPE_INT:
    case FIELD_TYPE_INT8:
    ...
    case FIELD_TYPE_INT32:
        sstr_printf_append(source,
            "    { int64_t _v; if (mp_unpack_number_as_int64(&_r, &_v) < 0) return -1;\n"
            "      %s = (%s)_v; }\n",
    ...
    case FIELD_TYPE_INT64:
        sstr_printf_append(source,
            "    { int64_t _v; if (mp_unpack_int64(&_r, &_v) < 0) return -1;\n"
            "      %s = _v; }\n", accessor);
        break;
    case FIELD_TYPE_UINT8:
    case FIELD_TYPE_UINT16:
    case FIELD_TYPE_UINT32:
        sstr_printf_append(source,
            "    { uint64_t _v; if (mp_unpack_number_as_uint64(&_r, &_v) < 0) return -1;\n"
            "      %s = (%s)_v; }\n",
            accessor, sstr_cstr(f->type_name));
        break;
    case FIELD_TYPE_UINT64:
        sstr_printf_append(source,
            "    { uint64_t _v; if (mp_unpack_uint64(&_r, &_v) < 0) return -1;\n"
            "      %s = _v; }\n", accessor);
        break;
    case FIELD_TYPE_FLOAT:
        sstr_printf_append(source,
            "    { double _v; if (mp_unpack_number_as_double(&_r, &_v) < 0) return -1;\n"
            "      %s = (float)_v; }\n", accessor);
        break;
    case FIELD_TYPE_DOUBLE:
        sstr_printf_append(source,
    ...
    case FIELD_TYPE_BOOL:
        sstr_printf_append(source,
            "    { int _v; if (mp_unpack_bool(&_r, &_v) < 0) return -1;\n"
            "      %s = _v; }\n", accessor);
        break;
    case FIELD_TYPE_SSTR:
        sstr_printf_append(source,
            "    { const char *_s; uint32_t _sl;\n"
            "      if (mp_unpack_str(&_r, &_s, &_sl) < 0) return -1;\n"
            "      sstr_clear(%s);\n"
    ...
    case FIELD_TYPE_ENUM:
        sstr_printf_append(source,
            "    { const char *_s; uint32_t _sl;\n"
            "      if (mp_peek(&_r) == MP_NIL) { mp_unpack_nil(&_r); %s = 0; }\n"
            "      else if ((unsigned char)_r.data[_r.pos] <= MP_FIXINT_MAX ||\n"
            "               ((unsigned char)_r.data[_r.pos] >= MP_NEG_FIXINT) ||\n"
            "               ((unsigned char)_r.data[_r.pos] >= MP_UINT8 && (unsigned char)_r.data[_r.pos] <= MP_INT64)) {\n"
            "          int64_t _v; if (mp_unpack_number_as_int64(&_r, &_v) < 0) return -1;\n"
            "          %s = (int)_v;\n"
            "      } else if (mp_unpack_str(&_r, &_s, &_sl) == 0) {\n"
            "          int _found = 0;\n"
    ...
    case FIELD_TYPE_STRUCT:
        sstr_printf_append(source,
            ...
            "      if (msgpack_unpack_%s(_r.data + _r.pos, _r.len - _r.pos, &%s) < 0) return -1;\n"
            "      /* advance reader past consumed bytes */\n"
            "      struct mp_reader _tmp; mp_reader_init(&_tmp, _r.data + _before, _r.len - _before);\n"
            "      if (mp_unpack_skip(&_tmp) < 0) return -1;\n"
    ...
    case FIELD_TYPE_ONEOF:
        ...
        break;
    default:
        sstr_append_cstr(source, "    mp_unpack_skip(&_r);\n");
        break;
    }
}

static void mp_gen_unpack_map_value(struct struct_field *field,
                                    const char *val_access, sstr_t source) {
    ...
    case FIELD_TYPE_STRUCT:
        mp_gen_unpack_scalar(&tmp, val_access, source);
        break;
    default:
        sstr_append_cstr(source, "            mp_unpack_skip(&_r);\n");
        break;
...
static void mp_gen_unpack_struct(struct struct_container *st, sstr_t source) {
    sstr_printf_append(source,
        "int msgpack_unpack_%s(const unsigned char *data, size_t len, struct %s *obj) {\n"
        "    struct mp_reader _r;\n"
        "    mp_reader_init(&_r, data, len);\n"
        "    uint32_t _nfields;\n"
        "    if (mp_unpack_map_header(&_r, &_nfields) < 0) return -1;\n"
        "    for (uint32_t _fi = 0; _fi < _nfields; _fi++) {\n"
        "        const char *_key; uint32_t _klen;\n"
        "        if (mp_unpack_str(&_r, &_key, &_klen) < 0) return -1;\n",
        sstr_cstr(st->name), sstr_cstr(st->name));
    ...
    while (f) {
        ...
        if (f->type == FIELD_TYPE_MAP && !f->is_array) {
            ...
            sstr_printf_append(source,
                "            { uint32_t _mc;\n"
                "              if (mp_unpack_map_header(&_r, &_mc) < 0) return -1;\n"
                ...
                "                  if (mp_unpack_str(&_r, &_mk, &_mkl) < 0) return -1;\n"
                "                  %s.entries[_mi].key = sstr_of(_mk, _mkl);\n",
                accessor, mp_map_suffix(f), mp_map_suffix(f),
                accessor, accessor);
            ...
            mp_gen_unpack_map_value(f, val_acc, source);
            sstr_append_cstr(source, "              }\n            }\n");
        } else if (f->is_array) {
            if (f->array_size > 0) {
                /* fixed array */
                sstr_printf_append(source,
                    "            { uint32_t _ac;\n"
                    "              if (mp_unpack_array_header(&_r, &_ac) < 0) return -1;\n"
                ...
                if (f->type == FIELD_TYPE_STRUCT) {
                    ...
                    sstr_t ct = sstr_new();
                    sstr_printf_append(ct, "struct map_container_%s", mp_map_suffix(f));
                    sstr_printf_append(source,
                        "            { uint32_t _ac;\n"
                        "              if (mp_unpack_array_header(&_r, &_ac) < 0) return -1;\n"
                        "              %s = (%s *)JGENC_MALLOC(sizeof(%s) * (_ac > 0 ? _ac : 1));\n"
...
emit_elem:;
            ...
            struct struct_field elem_f = *f;
            elem_f.is_array = 0;
            mp_gen_unpack_scalar(&elem_f, elem_acc, source);
            if (f->array_size > 0) {
                /* skip remaining if more than fixed size */
                sstr_printf_append(source,
                    "              }\n"
                    "              for (uint32_t _ai = %d; _ai < _ac; _ai++) mp_unpack_skip(&_r);\n"
                    "            }\n",
                    f->array_size);
            } else {
                sstr_append_cstr(source, "              }\n            }\n");
            ...
            sstr_printf_append(source, "    ");
            mp_gen_unpack_scalar(f, accessor, source);
        }
...
static void mp_gen_pack_array(struct struct_container *st, sstr_t source) {
    sstr_printf_append(source,
        "int msgpack_pack_array_%s(struct %s *obj, int count, sstr_t out) {\n"
        "    mp_pack_array_header(out, (uint32_t)count);\n"
        "    for (int _i = 0; _i < count; _i++) {\n"
        "        msgpack_pack_%s(&obj[_i], out);\n"
        "    }\n"
        "    return 0;\n}\n\n",
        sstr_cstr(st->name), sstr_cstr(st->name), sstr_cstr(st->name));
}

static void mp_gen_unpack_array(struct struct_container *st, sstr_t source) {
    sstr_printf_append(source,
        "int msgpack_unpack_array_%s(const unsigned char *data, size_t len, struct %s **obj, int *count) {\n"
        "    struct mp_reader _r;\n"
        "    mp_reader_init(&_r, data, len);\n"
        ...
        "        %s_init(&(*obj)[_i]);\n"
        "        size_t _before = _r.pos;\n"
        "        if (msgpack_unpack_%s(_r.data + _r.pos, _r.len - _r.pos, &(*obj)[_i]) < 0) {\n"
...
static void mp_gen_oneof_pack(struct oneof_container *oc, sstr_t source) {
    ...
    while (var) {
        sstr_printf_append(source,
            "    case %s_%s:\n"
            "        msgpack_pack_%s(&obj->value.%s, _tmp);\n"
            "        break;\n",
            sstr_cstr(oc->name), sstr_cstr(var->name),
            sstr_cstr(var->struct_type_name), sstr_cstr(var->name));
        i++;
    ...
    sstr_printf_append(source,
        ...
        "    struct mp_reader _cr;\n"
        "    mp_reader_init(&_cr, (const unsigned char *)sstr_cstr(_tmp), sstr_length(_tmp));\n"
        "    uint32_t _vc = 0;\n"
        "    if (sstr_length(_tmp) > 0) mp_unpack_map_header(&_cr, &_vc);\n"
        "    /* total = tag field + variant fields */\n"
        "    mp_pack_map_header(out, 1 + _vc);\n"
        "    /* pack tag */\n"
        "    mp_pack_str(out, \"%s\", %d);\n"
        "    if (obj->tag >= 0 && obj->tag < %s_tag_count) {\n"
        "        const char *_ts = %s_tag_strings[obj->tag];\n"
        "        mp_pack_str(out, _ts, (uint32_t)strlen(_ts));\n"
        "    } else {\n"
        "        mp_pack_int(out, (int64_t)obj->tag);\n"
        "    }\n"
...
static void mp_gen_oneof_unpack(struct oneof_container *oc, sstr_t source) {
    const char *tag_field = oc->tag_field ? sstr_cstr(oc->tag_field) : "type";
    ...
    sstr_printf_append(source,
        ...
        "    mp_reader_init(&_r, data, len);\n"
        "    uint32_t _nf;\n"
        "    if (mp_unpack_map_header(&_r, &_nf) < 0) return -1;\n"
        "    int _tag = -1;\n"
        "    /* Pass 1: find tag */\n"
        "    for (uint32_t _i = 0; _i < _nf; _i++) {\n"
        "        const char *_k; uint32_t _kl;\n"
        "        if (mp_unpack_str(&_r, &_k, &_kl) < 0) return -1;\n"
        "        if (_kl == %d && memcmp(_k, \"%s\", %d) == 0) {\n"
        "            const char *_ts; uint32_t _tsl;\n"
        "            if (mp_unpack_str(&_r, &_ts, &_tsl) < 0) return -1;\n"
    ...
    while (var) {
        sstr_printf_append(source,
            "    case %s_%s:\n"
            "        %s_init(&obj->value.%s);\n"
            "        if (msgpack_unpack_%s(data, len, &obj->value.%s) < 0) return -1;\n"
            "        break;\n",
            sstr_cstr(oc->name), sstr_cstr(var->name),
            sstr_cstr(var->struct_type_name), sstr_cstr(var->name),
            sstr_cstr(var->struct_type_name), sstr_cstr(var->name));
...
static void mp_gen_oneof_pack_array(struct oneof_container *oc, sstr_t source) {
    sstr_printf_append(source,
        "int msgpack_pack_array_%s(struct %s *obj, int count, sstr_t out) {\n"
        "    mp_pack_array_header(out, (uint32_t)count);\n"
        "    for (int _i = 0; _i < count; _i++) {\n"
...
static void mp_gen_oneof_unpack_array(struct oneof_container *oc, sstr_t source) {
    sstr_printf_append(source,
        "int msgpack_unpack_array_%s(const unsigned char *data, size_t len, struct %s **obj, int *count) {\n"
        "    struct mp_reader _r;\n"
        "    mp_reader_init(&_r, data, len);\n"
...
static void mp_gen_code_struct(struct struct_container *st, sstr_t source,
                               sstr_t header) {
    mp_gen_struct_header(st, header);
    mp_gen_struct_init(st, source);
    mp_gen_struct_clear(st, source);
    mp_gen_pack_struct(st, source);
    mp_gen_unpack_struct(st, source);
    mp_gen_pack_array(st, source);
    mp_gen_unpack_array(st, source);
}

static void mp_gen_code_oneof(struct oneof_container *oc, sstr_t source,
                              sstr_t header) {
    mp_gen_oneof_header(oc, header);
    mp_gen_oneof_init(oc, source);
    mp_gen_oneof_clear(oc, source);
    mp_gen_oneof_pack(oc, source);
    mp_gen_oneof_unpack(oc, source);
    mp_gen_oneof_pack_array(oc, source);
    mp_gen_oneof_unpack_array(oc, source);
}
...
static void mp_do_each_struct(void *key, void *value, void *ptr) {
    sstr_t k = (sstr_t)key;
    struct struct_container *st = (struct struct_container *)value;
    struct mp_gen_struct_param *p = (struct mp_gen_struct_param *)ptr;
    ...
    mp_gen_code_struct(st, p->source, p->header);
...
static void mp_do_each_oneof(void *key, void *value, void *ptr) {
    sstr_t k = (sstr_t)key;
    struct oneof_container *oc = (struct oneof_container *)value;
    struct mp_gen_oneof_param *p = (struct mp_gen_oneof_param *)ptr;
    ...
    mp_gen_code_oneof(oc, p->source, p->header);
    hash_map_insert(p->dependency_map, sstr_dup(k), NULL);
}

/* ── header preamble/epilogue ───────────────────────────────────────── */

static void mp_head_begin(sstr_t head) {
    sstr_append_cstr(head,
        "#ifndef MSGPACK_GEN_C_H__\n#define MSGPACK_GEN_C_H__\n\n"
        "#include \"sstr.h\"\n"
        "#include <stdbool.h>\n"
        "#include <stdint.h>\n"
        "#include <string.h>\n"
...
static void mp_head_end(sstr_t head) {
    sstr_append_cstr(head,
        "#ifdef __cplusplus\n}\n#endif\n\n"
        "#endif /* MSGPACK_GEN_C_H__ */\n");
}
...
static void mp_source_begin(sstr_t source) {
    sstr_append_cstr(source,
    ...
    /* Embed msgpack_codec.h + msgpack_codec.c */
    sstr_append_of(source, (const char *)msgpack_codec_h,
                   (size_t)msgpack_codec_h_len);
    sstr_append_cstr(source, "\n");
    sstr_append_of(source, (const char *)msgpack_codec_c,
                   (size_t)msgpack_codec_c_len);
    sstr_append_cstr(source, "\n");
}

/* ── public entry point ─────────────────────────────────────────────── */

int gencode_msgpack_source(struct hash_map *struct_map, struct hash_map *enum_map,
    ...
    /* header preamble */
    mp_head_begin(header);

    /* enum headers */
    hash_map_for_each(enum_map, mp_gen_enum_header_fn, header);

    /* source preamble (includes embedded runtime) */
    mp_source_begin(source);

    /* enum string arrays */
    hash_map_for_each(enum_map, mp_gen_enum_string_fn, source);

    /* oneof statics */
    hash_map_for_each(oneof_map, mp_gen_oneof_statics_fn, source);

    /* Generate structs + oneofs in dependency order */
    struct mp_gen_struct_param sp = { dep_map, source, header };
    struct mp_gen_oneof_param op = { dep_map, struct_map, source, header };
    int total = struct_map->size + oneof_map->size;

    for (int i = 0; i < total; i++) {
        hash_map_for_each(struct_map, mp_do_each_struct, &sp);
        hash_map_for_each(oneof_map, mp_do_each_oneof, &op);
